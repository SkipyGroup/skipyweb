...
function renderRuns(){const runs=state?.runs??[],signature=JSON.stringify(runs);if(signature===runsSignature)return;runsSignature=signature;const root=$('runs');root.replaceChildren();if(!runs.length){root.append(make('p','hint','Még nincs futási előzmény.'));return}for(const run of runs){const card=make('article',`run-card ${run.status}`);const head=make('div','run-head');head.append(make('strong','',run.name),make('span','',new Date(run.startedAt).toLocaleString('hu-HU')));const summary=make('p','',`${run.passed} sikeres · ${run.failed} hibás · ${run.skipped??0} kihagyva · ${(run.durationMs/1000).toFixed(1)} mp`);const details=document.createElement('details');const ds=document.createElement('summary');ds.textContent='Lépések részletei';details.append(ds);for(const result of run.results){const line=make('div',`run-step ${result.status}`,`${result.status==='passed'?'✓':result.status==='skipped'?'−':'✕'} ${result.name||result.id} · ${result.durationMs||0} ms${result.message?' · '+result.message:''}`);details.append(line)}const actions=make('div','run-actions');actions.append(button('Táblázatos riport',()=>void command('run-export',{id:run.id,format:'html'})),button('JSON adatok',()=>void command('run-export',{id:run.id,format:'json'})));card.append(head,summary,details,actions);root.append(card)}}
